...
namespace WolfstagInteractive.ConvoCore.SaveSystem.Tests
{
    /// <summary>
    /// EditMode coverage for Collection variables on <see cref="ConvoVariableStore"/>:
    /// round-trips, copy-on-write, events, serialization providers, and migration.
    /// </summary>
[HelpURL("https://docs.wolfstaginteractive.com/convocore/api/classWolfstagInteractive_1_1ConvoCore_1_1SaveSystem_1_1Tests_1_1ConvoVariableStoreCollectionTests.html")]
    public class ConvoVariableStoreCollectionTests
    {
        private ConvoVariableStore _store;

        [SetUp]
        public void SetUp()
        {
            _store = ScriptableObject.CreateInstance<ConvoVariableStore>();
        }
        ...
        private ConvoVariableEntry AddAuthoredIntCollection(string key, ConvoVariableScope scope,
            bool readOnly, params (string subKey, int value)[] pairs)
        {
            var variable = new ConvoCoreVariable { Key = key, Type = ConvoVariableType.CollectionInt };
            foreach (var (subKey, value) in pairs)
                variable.CollectionIntPairs.Add(new CollectionIntPair { SubKey = subKey, Value = value });

            var entry = new ConvoVariableEntry { CoreVariable = variable, Scope = scope, IsReadOnly = readOnly };
            _store.GetRawEntries().Add(entry);
            return entry;
        }

        private ConvoVariableEntry AddAuthoredStringCollection(string key, ConvoVariableScope scope,
            bool readOnly, params (string subKey, string value)[] pairs)
        {
            var variable = new ConvoCoreVariable { Key = key, Type = ConvoVariableType.CollectionString };
            foreach (var (subKey, value) in pairs)
        ...
        {
            foreach (ConvoVariableScope scope in System.Enum.GetValues(typeof(ConvoVariableScope)))
            {
        ...
        {
            var authored = AddAuthoredIntCollection("inventory", ConvoVariableScope.Global, false, ("sword", 1));

            _store.SetCollectionInt("inventory", "sword", 2, ConvoVariableScope.Global);
            _store.SetCollectionInt("inventory", "shield", 5, ConvoVariableScope.Global);
        ...
        {
            _store.SetCollectionInt("bag", "coin", 10, ConvoVariableScope.Global);
        ...
        {
            _store.SetCollectionInt("places", "town", 1, ConvoVariableScope.Global);
            _store.SetCollectionInt("places", "cave", 1, ConvoVariableScope.Global);
        ...
        {
            AddAuthoredIntCollection("inventory", ConvoVariableScope.Global, false, ("sword", 1));
            ...
            Assert.AreEqual(ConvoVariableType.CollectionInt, _store.GetVariable("inventory").Type);
            ...
            _store.SetCollectionInt("gold", "sub", 1, ConvoVariableScope.Global);
        ...
        {
            AddAuthoredIntCollection("locked", ConvoVariableScope.Global, true, ("gem", 1));

            LogAssert.Expect(LogType.Warning, new Regex("read-only"));
            _store.SetCollectionInt("locked", "gem", 99, ConvoVariableScope.Global);
        ...
        public void ResetVariable_RestoresAuthoredCollection_AndRemovesRuntimeCreated()
        {
            ...
            _store.SetCollectionInt("inventory", "sword", 9, ConvoVariableScope.Global);
            _store.SetCollectionInt("inventory", "shield", 4, ConvoVariableScope.Global);
            ...
            _store.SetCollectionInt("loot", "gem", 1, ConvoVariableScope.Global);
        ...
        public void MutatingOperations_FireExactlyOneEventWithDocumentedPayload()
        {
            ...
            _store.SetCollectionInt("inv", "sword", 2, ConvoVariableScope.Global);
            Assert.AreEqual(1, events.Count);
            Assert.AreEqual(("inv", (string)null, "2"), events[0]);

            _store.SetCollectionInt("inv", "sword", 3, ConvoVariableScope.Global);
            ...
            _store.SetCollectionInt("inv", "a", 1, ConvoVariableScope.Global);
            _store.SetCollectionInt("inv", "b", 2, ConvoVariableScope.Global);
            events.Clear();
            ...
            var authored = AddAuthoredIntCollection("flags", ConvoVariableScope.Global, false, ("seen", 1));
            Assert.IsFalse(authored.IsDirtySinceSnapshot);
            _store.SetCollectionInt("flags", "seen", 1, ConvoVariableScope.Global);
        ...
        {
            AssertProviderRoundTrip(new JsonFileConvoSaveProvider("ConvoCoreSaves_Tests"), "collections_json_test");
        }

        [Test]
        public void YamlProvider_RoundTrips_Collections()
        {
            AssertProviderRoundTrip(new YamlFileConvoSaveProvider("ConvoCoreSaves_Tests"), "collections_yaml_test");
        }

        private void AssertProviderRoundTrip(IConvoSaveProvider provider, string slot)
        {
            AddAuthoredStringCollection("relations", ConvoVariableScope.Global, false, ("elder", "friendly"));
            AddAuthoredIntCollection("emptyBag", ConvoVariableScope.Global, false);
            _store.SetCollectionString("relations", "guard", "hostile", ConvoVariableScope.Global);
            _store.SetCollectionInt("runtimeLoot", "gem", 7, ConvoVariableScope.Global);

            var snapshot = new ConvoCoreGameSnapshot
            {
                GlobalVariables = _store.ExportByScope(ConvoVariableScope.Global)
            };
            ...
            try
            {
                ...
                Assert.AreEqual(ConvoCoreGameSnapshot.CurrentSchemaVersion, loaded.SchemaVersion);

                var restoredStore = ScriptableObject.CreateInstance<ConvoVariableStore>();
                try
                {
        ...
        {
            var old = new ConvoCoreGameSnapshot { SchemaVersion = "1.0" };
            old.GlobalVariables.Add(new ConvoVariableEntry
            {
                CoreVariable = new ConvoCoreVariable { Key = "gold", Type = ConvoVariableType.Int }.SetInt(12),
                Scope = ConvoVariableScope.Global
            });

            var migrated = ConvoCoreSnapshotMigrator.Migrate(old);
        ...
        {
            AddAuthoredIntCollection("dupes", ConvoVariableScope.Global, false, ("sword", 1), ("sword", 9));
