...
{
    /// <summary>
    /// Node-graph authoring surface for a single <see cref="WitWeaverConversationData"/> asset.
    ...
    [HelpURL("https://docs.wolfstaginteractive.com/witweaver/api/classWolfstagInteractive_1_1WitWeaver_1_1GraphEditor_1_1WitWeaverConversationGraph.html")]
[Serializable]
    public class WitWeaverConversationGraph : Graph
    {
        public const string AssetExtension = "WitWeaverConversationGraph";

        [SerializeField] private WitWeaverConversationData m_Conversation;
        ...
        public override void OnEnable()
        {
            ...
            _loadedMutationStamp = WitWeaverGraphSync.GetMutationStamp(GraphDatabase.GetGraphAssetPath(this));
        }
        ...
        public override void OnGraphChanged(GraphLogger graphLogger)
        {
            ...
            if (!_reloadScheduled && !WitWeaverGraphSync.IsSyncInProgress)
            {
                var assetPath = GraphDatabase.GetGraphAssetPath(this);
                if (!string.IsNullOrEmpty(assetPath) &&
                    WitWeaverGraphSync.GetMutationStamp(assetPath) != _loadedMutationStamp)
                {
                    _reloadScheduled = true;
                    graphLogger.LogWarning("This graph changed on disk — the window is reloading.");
                    EditorApplication.delayCall += () => WitWeaverGraphBridge.ReloadWindowsForExternalChange(assetPath);
                    return;
                }
            }
            ...
            foreach (var node in GetNodes())
            {
                switch (node)
                {
                    ...
                    case PlayerChoiceNode choice:
                        ...
                        var inPort = choice.GetInputPortByName(WitWeaverGraphSchema.InPort);
            ...
            {
                var next = startNodes[0].GetOutputPortByName(WitWeaverGraphSchema.NextPort);
            ...
            {
                var next = line.GetOutputPortByName(WitWeaverGraphSchema.NextPort);
                if (next is { IsConnected: false })
            ...
            else
            {
                ...
                if (WitWeaverGraphSync.IsYamlStale(this, m_Conversation))
                    graphLogger.LogWarning(
        ...
        /// badge here, the matching refusal in <see cref="WitWeaverGraphBake.BuildModel"/>, and
        ...
        {
            if (WitWeaverGraphSync.IsSyncInProgress) return;

            var configs = WitWeaverGraphSync.GetYamlLineConfigs(m_Conversation);
        ...
        private static void ValidateContainerBranch(ContainerBranchNode branch, GraphLogger graphLogger)
        {
            ...
            var containerPort = branch.GetInputPortByName(WitWeaverGraphSchema.ContainerPort);
            containerPort?.TryGetValue(out container);

            string alias = null;
            branch.GetInputPortByName(WitWeaverGraphSchema.AliasOrNamePort)?.TryGetValue(out alias);
